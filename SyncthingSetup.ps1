...
$Devices = Import-Clixml -Path "$Env:OneDrive\Syncthing\Config\Devices.xml"
$Folders = Import-Clixml -Path "$Env:OneDrive\Syncthing\Config\Folders.xml"
...
{    
    if ($SyncthingConfig.folders.id -notcontains $Folder.Id)
    {
        if (! (Test-Path $folder.path -ErrorAction SilentlyContinue))
        {
            ...
            if (Test-Path "$Env:OneDrive\Syncthing\Config\stignore\${$folder.id}\.stignore" -ErrorAction SilentlyContinue)
            {
                Copy-Item -Path "$Env:OneDrive\Syncthing\Config\stignore\${$folder.id}\.stignore" -Destination "${$Folder.path}\.stignore" -Force
            }
        }
...
Get-SyncthingDevices | Export-Clixml -Path "$Env:OneDrive\Syncthing\Config\Devices.xml"
Get-SyncthingFolders | Export-Clixml -Path "$Env:OneDrive\Syncthing\Config\Folders.xml"
